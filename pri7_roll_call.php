...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($primary7 as $student) {
        $student_id = $student['id'];
        $status = isset($_POST['student_'.$student_id]) ? 'present' : 'absent';

        // Check for duplicate entries (same student, same date)
        $check_sql = "SELECT * FROM attendance7 WHERE student_id = $student_id AND date = '$attendance_date'";
        $check_result = $conn->query($check_sql);

        if ($check_result->num_rows === 0) {
            // If no duplicate, insert the attendance record
            $sql = "INSERT INTO attendance7 (student_id, date, status) VALUES ($student_id, '$attendance_date', '$status')";
            if (!$conn->query($sql)) {
