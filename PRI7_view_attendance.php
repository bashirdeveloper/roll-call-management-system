  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
        ...
        <li class="active"><a href="primaryseven.php">Primary Seven</a></li>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "SELECT a.*, s.first_name, s.last_name FROM attendance7 a JOIN primary7 s ON a.student_id = s.id WHERE 1=1";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $attendance7_records[] = $row;
        }
    } else {
        echo "No attendance7 records found.";
    }
}
...
<form action="" method="POST">
    ...
    <select name="student_id" id="student">
        ...
        <?php foreach ($primary7 as $student): ?>
...
<!-- Display attendance7 records -->
<?php if (!empty($attendance7_records)): ?>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <?php foreach ($attendance7_records as $record): ?>
                <tr>
...
<?php endif; ?>
<!-- //summary of primary7 present and absent -->
<?php
include('dbconnect.php');
$date_filter = isset($_POST['date']) ? $_POST['date'] : null;

// SQL query to count primary7 based on attendance7 status
if ($date_filter) {
    // For a specific date
    $sql_present = "SELECT COUNT(*) AS present_count FROM attendance7 WHERE status = 'present' AND date = '$date_filter'";
    $sql_absent = "SELECT COUNT(*) AS absent_count FROM attendance7 WHERE status = 'absent' AND date = '$date_filter'";
} else {
    // For all dates (overall summary)
    $sql_present = "SELECT COUNT(*) AS present_count FROM attendance7 WHERE status = 'present'";
    $sql_absent = "SELECT COUNT(*) AS absent_count FROM attendance7 WHERE status = 'absent'";
}

// Get the number of present primary7
$result_present = $conn->query($sql_present);
$row_present = $result_present->fetch_assoc();
$present_count = $row_present['present_count'];

// Get the number of absent primary7
