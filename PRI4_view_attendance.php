...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "SELECT a.*, s.first_name, s.last_name FROM attendance4 a JOIN primary4 s ON a.student_id = s.id WHERE 1=1";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $attendance4_records[] = $row;
        }
    } else {
        echo "No attendance4 records found.";
    }
}
...
<!-- Display attendance4 records -->
<?php if (!empty($attendance4_records)): ?>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <?php foreach ($attendance4_records as $record): ?>
                <tr>
...
<?php endif; ?>
<!-- //summary of primary4 present and absent -->
<?php
include('dbconnect.php');
$date_filter = isset($_POST['date']) ? $_POST['date'] : null;

// SQL query to count primary4 based on attendance4 status
if ($date_filter) {
    // For a specific date
    $sql_present = "SELECT COUNT(*) AS present_count FROM attendance4 WHERE status = 'present' AND date = '$date_filter'";
    $sql_absent = "SELECT COUNT(*) AS absent_count FROM attendance4 WHERE status = 'absent' AND date = '$date_filter'";
} else {
    // For all dates (overall summary)
    $sql_present = "SELECT COUNT(*) AS present_count FROM attendance4 WHERE status = 'present'";
    $sql_absent = "SELECT COUNT(*) AS absent_count FROM attendance4 WHERE status = 'absent'";
}

// Get the number of present primary4
$result_present = $conn->query($sql_present);
$row_present = $result_present->fetch_assoc();
$present_count = $row_present['present_count'];

// Get the number of absent primary4
