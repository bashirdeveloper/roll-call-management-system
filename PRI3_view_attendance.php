  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
        ...
        <li class="active"><a href="primarythree.php">Primary Three</a></li>
...
<div class="container-fluid">
  ...
  width: 80%;
      ...
      <ul class="nav nav-pills nav-stacked">
        ...
        <li ><a href="primaryone.php">Primary One</a></li>
        <li><a href="primarytwo.php">Primary Two</a></li>
        <li class="active"><a href="#">Primary Three</a></li>
        <li><a href="primaryfour.php">Primary Four</a></li>
        <li><a href="primaryfive.php">Primary Five</a></li>
        <li><a href="primarysix.php">Primary Six</a></li>
        <li><a href="primaryseven.php">Primary Seven</a></li>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "SELECT a.*, s.first_name, s.last_name FROM attendance3 a JOIN primary3 s ON a.student_id = s.id WHERE 1=1";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $attendance3_records[] = $row;
        }
    } else {
        echo "No attendance3 records found.";
    }
}
...
<form action="" method="POST">
    ...
    <select name="student_id" id="student">
        ...
        <?php foreach ($primary3 as $student): ?>
...
<!-- Display attendance3 records -->
<?php if (!empty($attendance3_records)): ?>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <?php foreach ($attendance3_records as $record): ?>
                <tr>
...
<?php endif; ?>
<!-- //summary of primary3 present and absent -->
<?php
include('dbconnect.php');
$date_filter = isset($_POST['date']) ? $_POST['date'] : null;

// SQL query to count primary3 based on attendance3 status
if ($date_filter) {
    // For a specific date
    $sql_present = "SELECT COUNT(*) AS present_count FROM attendance3 WHERE status = 'present' AND date = '$date_filter'";
    $sql_absent = "SELECT COUNT(*) AS absent_count FROM attendance3 WHERE status = 'absent' AND date = '$date_filter'";
} else {
    // For all dates (overall summary)
    $sql_present = "SELECT COUNT(*) AS present_count FROM attendance3 WHERE status = 'present'";
    $sql_absent = "SELECT COUNT(*) AS absent_count FROM attendance3 WHERE status = 'absent'";
}

// Get the number of present primary3
$result_present = $conn->query($sql_present);
$row_present = $result_present->fetch_assoc();
$present_count = $row_present['present_count'];

// Get the number of absent primary3
