  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
        <li class="active"><a href="homepage.php">RCM System</a></li>
        <li ><a href="primaryone.php">Primary One</a></li>
        <li class="active"><a href="primarytwo.php">Primary Two</a></li>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "SELECT a.*, s.first_name, s.last_name FROM attendance2 a JOIN primary2 s ON a.student_id = s.id WHERE 1=1";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $attendance2_records[] = $row;
        }
    } else {
        echo "No attendance2 records found.";
    }
}
...
<form action="" method="POST">
    ...
    <select name="student_id" id="student">
        ...
        <?php foreach ($primary2 as $student): ?>
...
<!-- Display attendance2 records -->
<?php if (!empty($attendance2_records)): ?>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <?php foreach ($attendance2_records as $record): ?>
                <tr>
...
<?php endif; ?>
<!-- //summary of primary2 present and absent -->
<?php
include('dbconnect.php');
$date_filter = isset($_POST['date']) ? $_POST['date'] : null;

// SQL query to count primary2 based on attendance2 status
if ($date_filter) {
    // For a specific date
    $sql_present = "SELECT COUNT(*) AS present_count FROM attendance2 WHERE status = 'present' AND date = '$date_filter'";
    $sql_absent = "SELECT COUNT(*) AS absent_count FROM attendance2 WHERE status = 'absent' AND date = '$date_filter'";
} else {
    // For all dates (overall summary)
    $sql_present = "SELECT COUNT(*) AS present_count FROM attendance2 WHERE status = 'present'";
    $sql_absent = "SELECT COUNT(*) AS absent_count FROM attendance2 WHERE status = 'absent'";
}

// Get the number of present primary2
$result_present = $conn->query($sql_present);
$row_present = $result_present->fetch_assoc();
$present_count = $row_present['present_count'];

// Get the number of absent primary2
